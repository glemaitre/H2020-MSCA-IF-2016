...
% The WP1 for ``Data acquisition and dissemination'' is dedicated to work together with clinicians to collect the prostate mp-MRI (0.125 CY), generate the associated ground-truth (0.08 CY).
% Additionally, the dataset will be anonymized and deidentified (0.04) such that it can be proposed as Open Data for a Grand-Challenge.
% The WP2 for ``Pre-processing'' is dedicated to study to evaluate bias correction algorithms (0.125 CY), denoising methods (0.125 CY), develop a normalization method for \ac{dw}-\ac{mri} modality (0.17 CY), and study pre-processing method for \ac{mrsi} (0.17 CY).
% The WP3 for ``Segmentation'' is dedicated to the development and evaluation of an hybrid segmentation method on the public dataset (0.58 CY).
% The WP4 for ``Registration'' is dedicated to extend the current registration method and validate this method on the public dataset (0.58 CY).
% The WP5 for ``Detection and assessment'' is dedicated to the development of a deep-learning based prostate cancer detection system as well as is evaluation on the public dataset  (0.58 CY).
% The WP6 for `` PI-RADS evaluation'' is dedicated to translate the classification results to a PI-RADS evaluation (0.17 CY).
